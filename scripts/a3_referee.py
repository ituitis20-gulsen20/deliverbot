...
class Referee(Node):
    ...
    def __init__(self):
        ...
        self.pkg_share_dir = get_package_share_directory('assignment3')
        self.pkg_dir = get_package_prefix('assignment3')
        self.pkg_dir = self.pkg_dir.split("install")[0] + "src/assignment3"
        self.pkg_wp_report_dir = os.path.join(self.pkg_dir,"maps","wpReport.txt")
        self.pkg_time_report_dir = os.path.join(self.pkg_dir,"maps","timeReport.txt") 
        self.waypoints_path = os.path.join(self.pkg_share_dir,"maps","waypoints")
    ...
    def odom_callback(self, msg):
        ...
        if ((round(msg.twist.twist.linear.x,2) > 0.0) | (round(msg.twist.twist.angular.z,2) > 0.0)):
            ...
            if (self.move_counter==0):
                ...
                with open(self.pkg_wp_report_dir, 'r+') as f:
                    f.truncate(0)
                f.close()   
            self.move_counter+=1
        if (self.waypoint_index!=18):
            ...
            if linear_error<=XY_CLOSENESS_THRESH:
                ...
                with open(self.pkg_wp_report_dir, 'a') as f:
                    f.write(wp_indicator)
                    f.write(","+str(duration_error))
                    f.write("\n")
                f.close()
                self.waypoint_index+=1
            with open(self.pkg_time_report_dir,'w') as f:
